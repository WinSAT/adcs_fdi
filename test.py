...
print ("ended data conversion to 2d array for split into train yvalues")
...
# Training classifiers
clf1 = AdaBoostClassifier(DecisionTreeClassifier(max_depth=None))
#clf2 = AdaBoostClassifier(KNeighborsClassifier(n_neighbors=10))
clf3 = AdaBoostClassifier(SVC(gamma='scale', kernel='rbf', probability=True))
...
try:
	...
	algList = [eclf1,eclf3]
	predList = [y_pred1,y_pred3]
	from IPython import embed; embed()
	for idx,alg in enumerate([eclf1,eclf3]):
		scores = cross_val_score(alg, xValues, yValues, cv=5)
		print("%s Accuracy: %0.2f (+/- %0.2f)" % (str(idx),scores.mean(), scores.std() * 2))
		print("accuracy_score:", accuracy_score(predList[idx], y_test))
		print(metrics.confusion_matrix(y_test.T[0], predList[idx].T[0], labels=range(16)))
		print(metrics.confusion_matrix(y_test, predList[idx], labels=range(16)))
		print("Scenario accuracy: "+str(np.sum(y_test==predList[idx])/len(y_test)*100)+"%")
		print("Scenario accuracy: "+str(np.sum(y_test.T[0]==predList[idx].T[0])/len(y_test.T[0])*100)+"%")
except:
	from IPython import embed; embed()
from IPython import embed; embed()
